...
static inline bool getAddressFromString4(const std::string& ip, sockaddr_in& addr) {
    addrinfo* _addrinfo;
    bool good    = false;
...
static inline void closeSocket(socket_t socket) {
    ::closesocket(socket);
}
...
static inline void closeNetwork() {
    WSACleanup();
}
...
#define SEND_FLAGS MSG_NOSIGNAL
    ...
    *(uint32_t *)&m_buffer[0] = (uint32_t)m_index; // write size
    ...
    while(total < m_index && s_socket) {
        stream_size_t ret = ::send(s_socket->Socket, (const char*)m_buffer.data() + total, (int)(m_index - total), SEND_FLAGS);
...
Streaming::Streaming(const Image &image)
: m_image(image) {
...
void Streaming::updateBlock(const Bounds2i &block) {
    ...
    for (int channel = 0; channel < Color::NumComponents; channel++) {
        ...
        for (auto pixel : block) data.push_back(m_image(pixel)[channel] * m_normalization);
...
void Streaming::update() {
    // we need to split up the image into smaller packets, since large ones are not supported
...
void Streaming::stopRegularUpdates() {
    m_updater = nullptr;
}
