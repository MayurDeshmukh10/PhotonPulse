...
/**
 * @brief Parent class for shapes that combine many individual shapes (e.g., triangle meshes), and hence
 * benefit from building an acceleration structure over their children.
 * 
 * To use this class, you will need to implement the following methods:
 * - numberOfPrimitives()           -- report the number of individual children that the shape has
 * - intersect(primitiveIndex, ...) -- intersect a single child (identified by the given index) for the given ray
 * - getBoundingBox(primitiveIndex) -- return the bounding box of a single child (used for building the BVH)
 * - getCentroid(primitiveIndex)    -- return the centroid of a single child (used for building the BVH)
 * 
 * @example For a simple example of how to use this class, look at @ref shapes/group.cpp
 * @see Group
 * @see TriangleMesh
 */
class AccelerationStructure : public Shape {
    ...
    struct Node {
        ...
        Bounds aabb;
        /**
         * @brief Either the index of the left child node in m_nodes (for internal nodes), or the first primitive
         * in m_primitiveIndices (for leaf nodes).
         * @note For efficiency, we store the BVH nodes so that the right child always directly follows the left
         * child, i.e., the index of the right child is always @code leftFirst + 1 @endcode .
         * @note For efficiency, we store primitives so that children of a leaf node are always contigous in
         * m_primitiveIndices.
         */
        NodeIndex leftFirst;
        /// @brief The number of primitives in a leaf node, or 0 to indicate that this node is not a leaf node.
        NodeIndex primitiveCount;

        /// @brief Whether this BVH node is a leaf node.
        bool isLeaf() const { return primitiveCount != 0; }

        /// @brief For internal nodes: The index of the left child node in m_nodes. 
        NodeIndex leftChildIndex() const { return leftFirst; }
        /// @brief For internal nodes: The index of the right child node in m_nodes.
        ...
        /// @brief For leaf nodes: The last index in m_primitiveIndices (still included).
        NodeIndex lastPrimitiveIndex() const { return leftFirst + primitiveCount - 1; }
    };

    /// @brief A list of all BVH nodes.
    std::vector<Node> m_nodes;
    /**
     * @brief Mapping from internal @c NodeIndex to @c primitiveIndex as used by all interface methods.
     * For efficient storage, we assume that children of BVH leaf nodes have contiguous indices, which would
     * require re-ordering the primitives. For simplicity, we instead perform this re-ordering on a list of indices
     * (which starts of as @code 0, 1, 2, ..., primitiveCount - 1 @endcode ), which allows us to translate from
     * re-ordered (contiguous) indices to the indices the user of this class expects.
     */
    ...
    /**
     * @brief Intersects a BVH node, recursing into children (for internal nodes), or intersecting all
     * primitives (for leaf nodes).
     */
    bool intersectNode(const Node &node, const Ray &ray, Intersection &its, Sampler &rng) const {
        // update the statistic tracking how many BVH nodes have been tested for intersection
        its.stats.bvhCounter++;

        bool wasIntersected = false;
        if (node.isLeaf()) {
            for (NodeIndex i = 0; i < node.primitiveCount; i++) {
                // update the statistic tracking how many children have been tested for intersection
            ...
            // this allows us to traverse the children in the order they are intersected in, which can
            // help prune a lot of unnecessary intersection tests.
            ...
            if (leftT < rightT) { // left child is hit first; test left child first, then right child
                if (leftT  < its.t) wasIntersected |= intersectNode(m_nodes[node.leftChildIndex() ], ray, its, rng);
                if (rightT < its.t) wasIntersected |= intersectNode(m_nodes[node.rightChildIndex()], ray, its, rng);
    ...
    /// @brief Performs a slab test to intersect a bounding box with a ray, returning Infinity in case the ray misses.
    float intersectAABB(const Bounds &bounds, const Ray &ray) const {
        ...
        // the elementwiseMin picks the near slab for each axis, of which we then take the maximum
        const auto tNear = elementwiseMin(t1, t2).maxComponent();
        // the elementwiseMax picks the far slab for each axis, of which we then take the minimum
        const auto tFar = elementwiseMax(t1, t2).minComponent();

        if (tFar < tNear) return Infinity; // the ray does not intersect the bounding box
        if (tFar < Epsilon) return Infinity; // the bounding box lies behind the ray origin
        
        return tNear; // return the first intersection with the bounding box (may also be negative!)
    }
    ...
    void subdivide(Node &parent) {
        ...
        // the point at which to split (note that primitives must be re-ordered so that all
        // children of the left node will have a smaller index than i, and nodes on the right will have
        // an index larger or equal to i)
...
protected:
    /// @brief  Returns the number of children (individual shapes) that are part of this acceleration structure.
    virtual int numberOfPrimitives() const = 0;
    /// @brief Intersect a single child (identified by the index) with the given ray.
    ...
    void buildAccelerationStructure() {
        ...
        logger(EInfo, "built BVH with %ld nodes for %ld primitives in %.1f ms",
            m_nodes.size(), numberOfPrimitives(), buildTimer.getElapsedTime() * 1000);
    }

public:
    bool intersect(const Ray &ray, Intersection &its, Sampler &rng) const override {
        if (m_primitiveIndices.empty()) return false; // exit early if no children exist
