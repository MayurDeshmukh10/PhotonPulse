...
struct BsdfSample {
    ...
    Vector wi;
    /// @brief The weight of the sample, given by @code cos(theta) * B(wi, wo) / p(wi) @endcode
    Color weight;
...
public:
    /**
     * @brief Samples a direction according to the distribution of the Bsdf in local coordinates (i.e., the normal is assumed to be [0,0,1]).
     * @note Can also produce invalid samples in case sampling fails (use @ref BsdfSample::isInvalid() to check for this).
     * @param uv The texture coordinates of the surface.
     * @param wo The outgoing direction light is scattered in, pointing away from the surface, in local coordinates.
