...
class Logger {
    /// @brief Synchronization to ensure that messages from different threads are not intermangled.
    std::mutex m_mutex;
    /// @brief A status message to be shown at the bottom of console output (e.g., render progress in percent).
    std::string m_status;

public:
    /// @brief Logs a message to console output, which will be constructed from the given format string and corresponding arguments.
    ...
    void operator()(LogLevel level, const char* fmt, const Args&... args) {
        ...
        switch (level) {
        case EDebug: std::cout << "\033[90m[debug] \033[0m"; break;
        case EInfo:  std::cout << "\033[32m[info] \033[0m"; break;
        case EWarn:  std::cout << "\033[33m[warn] \033[0m"; break;
        case EError: std::cout << "\033[31m[error] \033[0m"; break;
        }
        (level >= EError ? std::cerr : std::cout) << tfm::format(fmt, args...) << std::endl;

        std::cout << m_status << std::flush;
    }

    /// @brief Sets the status text for display at the bottom of console output, constructed from a given format string.
    template<typename... Args>
...
/// @brief A convenience class to keep the user updated about the progress of a long-running task.
class ProgressReporter {
    /// @brief The number of work units that need to be completed for the task to finish.
    int m_unitsTotal;
    ...
    std::string makeProgressBar(float progress, int width = 32) {
        int index = round(progress * width);
        std::string result = "\033[96m";
        for (int i = 0; i < width; i++) {
            if (i == index) result += "\033[90m";
...
public:
    ...
    /// @brief Marks a number of @c unitsCompleted as completed and notifies the user about the progress.
    void operator+=(int unitsCompleted) {
        ...
        logger.setStatus(
            "\033[96m[render]\033[0m %s \033[96m%3.0f%%\033[0m (\033[92m%.0fs\033[0m elapsed, \033[93m%.0fs\033[0m eta)",
